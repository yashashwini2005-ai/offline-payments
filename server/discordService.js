...
const sendDiscordNotification = async ({ transactionId, amount, merchant, status, timestamp }) => {
  ...
  // Format timestamp to IST-friendly string
  ...
  const embed = {
    ...
    footer: {
      text: 'JanPay Secure Settlement Engine • POST /notify'
    },
    fields: [
  ...
  try {
    ...
    // Silent failure — never crash the API server
    console.error('[Discord] Network error when sending webhook:', err.message);
  }
};
